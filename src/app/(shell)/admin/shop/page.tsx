...
import { listAdminShopAcquisitions } from "@/server/services/adminControl";
...
export default async function AdminShopPage({ searchParams }: PageProps) {
  ...
  const items = await listAdminShopAcquisitions(120, normalizedUser);

  return (
    <AdminSectionCard
      eyebrow="Acquisition Feed"
    ...
    >
      {items.length === 0 ? (
        <div className="text-sm text-neutral-500">
          {normalizedUser ? "No purchases found for that player." : "No purchases yet."}
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="text-sm text-neutral-200">
                    {row.user.profile?.displayName ?? `@${row.user.username}`} acquired {row.item.title}
                  </div>
                  <div className="mt-1 text-xs uppercase tracking-[0.16em] text-neutral-500">
                    {row.item.category} · €$ {row.item.priceEurodollars}
                  </div>
                </div>
